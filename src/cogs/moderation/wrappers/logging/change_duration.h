...
class change_duration final: public simple_wrapper {
	std::string case_id;
	dpp::message response;
	std::string old_duration;
...
public:
	 ...
	 */
	change_duration(moderation_command command, std::string case_id): simple_wrapper(std::move(command)), case_id(std::move(case_id)){}
};
