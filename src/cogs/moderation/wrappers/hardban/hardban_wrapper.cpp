...
void hardban_wrapper::check_hardban_possible(shared_vector<dpp::role> const& protected_roles) {
	...
	for(auto const &member: members) {
		...
		if(member_top_role->position > bot_top_role->position) {
			errors.push_back(std::format("❌ {} has a higher role than the bot. Unable to hard ban. "
...
void hardban_wrapper::check_permissions() {
	...
	check_hardban_possible(protected_roles);
...
void hardban_wrapper::process_response() {
	...
	if(command.interaction) {
		...
		log_command_invoke(internal::mod_action_name::HARD_BAN);
	}
	else
