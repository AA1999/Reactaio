	...
	for (auto* user : users) {
		if (command.interaction) { // If this is automod, DMing lots of users WILL result in a ratelimit
			...
			if(duration) {
				...
				dm_message = fmt::format("You have been banned by from {} by {} until {}. Reason: {}", command.guild->name,
				                         author_user->format_username(), time_future_relative, command.reason);
			}
			else
				dm_message = fmt::format("You have been banned from {} by {}. Reason: {}.", command.guild->name,
		...
		command.bot->guild_ban_add(command.guild->id, user->id, ban_remove_days , [this, user](auto const& completion) {
			if (completion.is_error()) {
				...
				errors.push_back(fmt::format("❌ Unable to ban user **{}**. Error code {}: {}.",
				...
				if(duration) {
					...
					auto future_str = fmt::format("{}", future);
...
void ban_wrapper::process_response() {
	...
	if (!are_all_errors()) {
		...
		std::transform(banned_users.begin(), banned_users.end(), std::back_inserter(banned_usernames),
		               [=](dpp::user const* user) {
			               return fmt::format("**{}**", user->format_username());
		               });
		...
		if (banned_users.size() == 1) {
			title		= "Banned";
			description = fmt::format("{} has been banned", usernames);
			gif_url		= "https://media3.giphy.com/media/LOoaJ2lbqmduxOaZpS/giphy.gif";
		}
		else {
			title		= "Banned";
			description = fmt::format("{} have been banned", usernames);
		...
		if(duration) {
			...
			std::string time_future_relative = dpp::utility::timestamp(future.time_since_epoch().count(),
			                                                           dpp::utility::time_format::tf_relative_time);
			description.append(fmt::format(" until {}.", time_future_relative));
		}
		else
			description.append(".");
	...
	for(auto const& member: members) {
		...
		if(!protected_roles.empty()) {
			...
			if(!member_protected_roles.empty()) { // If member has any of the protected roles.
				...
				errors.push_back(fmt::format("Member has the protected roles: {}. Cannot ban.", role_mentions_str));
			}
		}

		if(member_top_role->position > bot_top_role->position) {
			errors.push_back(fmt::format("❌ {} has a higher role than the bot. Unable to ban. Please "
		...
		if(member_top_role->position > author_top_role->position) {
			errors.push_back(fmt::format("❌ {} has a higher role than you do. You can't ban them.",
			                                member.get_mention()));
			cancel_operation = true;
		}
	}
